...
def parse(nextpage=None):
    try:
        ...
        if nextpage:
            payload = {'pageToken': nextpage, 'maxResults': 500}
        ...
        for item in tqdm(data['messages']):
            try:
                cur.execute('select exists(select 1 from gmail where id_msg = ? LIMIT 1)', (item['id'],))
                record = cur.fetchone()
                if record[0] == 1:
                    print('Message already saved', item['id'])
                else:
